...
_metrics_utils_spec = importlib.util.spec_from_file_location(
    "local_metrics_utils", _metrics_utils_path
)
...
def _create_extended_log_entry(
    base_entry: dict, state: PhaseState, model, cfg: Config
) -> dict:
    ...
    if hasattr(model, "odometer") and model.odometer:
        try:
            if (
                hasattr(model.odometer, "sigma_step")
                and model.odometer.sigma_step is not None
            ):
                odometer_metrics["sigma_step_theory"] = model.odometer.sigma_step
            if (
                hasattr(model.odometer, "eps_step")
                and model.odometer.eps_step is not None
            ):
                odometer_metrics["eps_step_theory"] = model.odometer.eps_step
            if (
                hasattr(model.odometer, "delta_step")
                and model.odometer.delta_step is not None
            ):
                odometer_metrics["delta_step_theory"] = model.odometer.delta_step
            if hasattr(model.odometer, "rho_total") and hasattr(
                model.odometer, "deletion_capacity"
            ):
                if model.odometer.deletion_capacity > 0:
                    odometer_metrics["rho_step"] = (
                        model.odometer.rho_total / model.odometer.deletion_capacity
                    )
        except Exception:
    ...
    if comp_type in (None, "none", "", np.nan):
        try:
            if getattr(cfg, "enable_oracle", False):
                comp_type = getattr(cfg, "comparator", "dynamic")
            else:
                comp_type = "none"
        except Exception:
            comp_type = "none"
    ...
    entry.update(
        {
            ...
            "lambda_est": model_metrics.get(
                "lambda_est", getattr(model, "lambda_est", None)
            ),
            ...
            "sigma_step": model_metrics.get(
                "sigma_step", odometer_metrics.get("sigma_step_theory", None)
            ),
            ...
            "P_T_est": model_metrics.get(
                "P_T_est", model_metrics.get("P_T", None)
            ),  # Legacy alias
    ...
    try:
        if (
            ("regret" not in entry)
            or (entry["regret"] is None)
            or (
                isinstance(entry["regret"], float) and np.isnan(entry["regret"])  # type: ignore[arg-type]
            )
        ):
            if (
                "regret_increment" in model_metrics
                and model_metrics["regret_increment"] is not None
            ):
                entry["regret"] = model_metrics["regret_increment"]
        else:
            inc = model_metrics.get("regret_increment")
            if inc is not None:
                entry["regret"] = inc
    except Exception:
        pass
...
def bootstrap_phase(
    ...
    print(
        f"[Bootstrap] Collecting {cfg.bootstrap_iters} steps to estimate G, D, c, C..."
    )
    ...
    for _ in range(cfg.bootstrap_iters):
        ...
        pred, grad = get_pred_and_grad(
            model, state.current_x, state.current_y, is_calibration=True
        )
...
def sensitivity_calibration_phase(
    ...
    for _ in range(cfg.sens_calib):
        ...
        if hasattr(model, "estimate_delete_sensitivity"):
            sensitivity = model.estimate_delete_sensitivity(
...
def warmup_phase(
    ...
    # Freeze static comparator (if enabled) right after warmup
    try:
        if (
            getattr(cfg, "enable_oracle", False)
            and getattr(cfg, "comparator", "dynamic") == "static"
        ):
            if (
                hasattr(model, "oracle")
                and model.oracle is not None
                and hasattr(model.oracle, "finalize")
            ):
                model.oracle.finalize()
                print("[Comparator] StaticOracle finalized (frozen w*)")
    except Exception as _e:
        print(f"[Comparator] Warning: failed to finalize StaticOracle: {_e}")
    ...
    while events_used < max_events_left:
        try:
            ...
            if k_inserts < cfg.delete_ratio:
                ...
                # Create extended log entry with new columns
                log_entry = _create_extended_log_entry(
                    base_log_entry, state, model, cfg
                )
