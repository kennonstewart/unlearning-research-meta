...
def load_seed_summaries(sweep_dir: str) -> pd.DataFrame:
    ...
    grid_dirs = [d for d in os.listdir(sweep_dir) 
...
def normalize_types(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_cols = [
        ...
        'gamma_insert_threshold', 'gamma_delete_threshold', 'gamma_error',
        'PT_error', 'ST_error', 'rho_spent_final', 'rho_total', 'delete_ratio',
        'max_g_norm', 'target_G', 'avg_clip_rate', 'target_PT', 'target_ST'
    ]
...
def compute_at_clean_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'max_g_norm' in df.columns and 'target_G' in df.columns and 'avg_clip_rate' in df.columns:
    ...
    df['AT_clean_overall'] = (df['AT1_clean'] & df['AT2_clean'] & 
                             df['AT5_clean'] & df['AT6_clean'])
    
    return df


def load_event_logs(sweep_dir: str, grid_id: str, max_seeds: int = 4) -> Dict[int, pd.DataFrame]:
    """
...
def plot_regret_time_series_with_phases(sweep_dir: str, grid_ids: List[str], 
                                       gamma_bar: float, max_seeds: int = 4,
    ...
    fig, axes = plt.subplots(n_grids, 1, figsize=figsize, 
    ...
    for i, grid_id in enumerate(grid_ids):
        ...
        if not event_data:
            ax.text(0.5, 0.5, f'No event data for {grid_id}', 
        ...
        for j, (seed, df) in enumerate(event_data.items()):
            ...
            ax.plot(x_vals, avg_regret_series, color=colors[j], 
        ...
        # Add gamma bar line
        ax.axhline(y=gamma_bar, color='red', linestyle='--', linewidth=2,
...
def add_phase_overlays(ax, df: pd.DataFrame, x_vals: pd.Series) -> None:
    ...
    for i, op in enumerate(df['op']):
        if op != current_op:
            ...
            if current_op is not None and i > start_idx:
                ...
                if current_op in ['insert', 'delete']:
                    color = 'lightcyan'  # Group interleaving phases
                ax.axvspan(x_vals.iloc[start_idx], x_vals.iloc[i-1], 
                          alpha=0.2, color=color, label=f'{current_op}' if current_op not in ax.get_legend_handles_labels()[1] else "")
    ...
    if current_op is not None and len(df) > start_idx:
        ...
        if current_op in ['insert', 'delete']:
            color = 'lightcyan'
        ax.axvspan(x_vals.iloc[start_idx], x_vals.iloc[-1], 
                  alpha=0.2, color=color, label=f'{current_op}' if current_op not in ax.get_legend_handles_labels()[1] else "")


def plot_regret_ratio_ecdf(df: pd.DataFrame, at_clean_subset: Optional[pd.DataFrame] = None,
    ...
    if 'regret_ratio' in df.columns:
        ...
        if len(ratios) > 0:
            ...
            ax.plot(sorted_ratios, y_vals, label=f'All seeds (n={len(ratios)})', 
    ...
    if at_clean_subset is not None and 'regret_ratio' in at_clean_subset.columns:
        ...
        if len(ratios_clean) > 0:
            ...
            y_vals_clean = np.arange(1, len(sorted_ratios_clean) + 1) / len(sorted_ratios_clean)
            ax.plot(sorted_ratios_clean, y_vals_clean, 
                   label=f'AT-clean subset (n={len(ratios_clean)})', 
                   linewidth=2, alpha=0.8, linestyle='--')
    
    # Add r=1 threshold line
    ax.axvline(x=1.0, color='red', linestyle='--', linewidth=2, label='r = 1 (pass threshold)')
...
def plot_regret_ratio_by_grid(df: pd.DataFrame, top_n: int = 20, 
    ...
    grid_stats = df.groupby('grid_id')['regret_ratio'].agg(['median', 'count']).reset_index()
    ...
    # Create violin plot
    sns.violinplot(data=plot_data, y='grid_id', x='regret_ratio', 
                   order=worst_grids['grid_id'], ax=ax, inner='box')
...
def plot_pass_rate_by_grid(df: pd.DataFrame, top_n: int = 20,
                          figsize: Optional[Tuple[int, int]] = None) -> None:
    """
    ...
    pass_rates = df.groupby('grid_id')['gamma_pass_overall'].agg(['mean', 'count']).reset_index()
    ...
    # Create bar plot
    bars = ax.barh(range(len(pass_rates)), pass_rates['pass_rate'], 
    ...
    for i, (idx, row) in enumerate(pass_rates.iterrows()):
        ax.text(row['pass_rate'] + 0.01, i, f'{row["pass_rate"]:.2f} ({row["total_seeds"]})', 
...
def plot_pass_rate_heatmaps(df: pd.DataFrame) -> None:
    ...
    if all(col in df.columns for col in ['gamma_bar_threshold', 'gamma_split_threshold', 'gamma_pass_overall']):
        heatmap_data1 = df.groupby(['gamma_bar_threshold', 'gamma_split_threshold'])['gamma_pass_overall'].mean().unstack()
        if not heatmap_data1.empty:
            sns.heatmap(heatmap_data1, annot=True, fmt='.2f', cmap='RdYlGn', 
                       ax=axes[0], cbar_kws={'label': 'Pass Rate'})
            axes[0].set_title('Pass Rate by (γ̄, γ_split)')
            axes[0].set_xlabel('gamma_split_threshold')
            axes[0].set_ylabel('gamma_bar_threshold')
        else:
            axes[0].text(0.5, 0.5, 'No data for gamma thresholds', ha='center', va='center', transform=axes[0].transAxes)
    else:
        axes[0].text(0.5, 0.5, 'Missing gamma threshold columns', ha='center', va='center', transform=axes[0].transAxes)
    
    # Heatmap 2: rho_total vs delete_ratio
    if all(col in df.columns for col in ['rho_total', 'delete_ratio', 'gamma_pass_overall']):
        heatmap_data2 = df.groupby(['rho_total', 'delete_ratio'])['gamma_pass_overall'].mean().unstack()
        if not heatmap_data2.empty:
            sns.heatmap(heatmap_data2, annot=True, fmt='.2f', cmap='RdYlGn', 
            ...
            axes[1].text(0.5, 0.5, 'No data for rho/delete ratio', ha='center', va='center', transform=axes[1].transAxes)
    else:
        axes[1].text(0.5, 0.5, 'Missing rho_total or delete_ratio columns', ha='center', va='center', transform=axes[1].transAxes)
    
    plt.tight_layout()
    plt.show()


def plot_regret_vs_gamma_scatter(df: pd.DataFrame, figsize: Tuple[int, int] = (10, 8)) -> None:
    """
    Plot scatter of avg_regret_final vs γ̄ with y=x reference and pass/fail coloring.
    
    Args:
        df: DataFrame with gamma metrics
        figsize: Figure size
    """
    if not all(col in df.columns for col in ['avg_regret_final', 'gamma_bar_threshold', 'gamma_pass_overall']):
        print("Missing required columns: avg_regret_final, gamma_bar_threshold, gamma_pass_overall")
        return
    
    fig, ax = plt.subplots(figsize=figsize, facecolor='white')
    
    # Prepare data
    plot_df = df.dropna(subset=['avg_regret_final', 'gamma_bar_threshold', 'gamma_pass_overall'])
    ...
    # Color by pass/fail
    colors = ['red' if not passed else 'green' for passed in plot_df['gamma_pass_overall']]
    
    # Create scatter plot
    scatter = ax.scatter(plot_df['gamma_bar_threshold'], plot_df['avg_regret_final'], 
                        c=colors, s=sizes, alpha=0.6, edgecolors='black', linewidth=0.5)
    
    # Add y=x reference line
    min_val = min(plot_df['gamma_bar_threshold'].min(), plot_df['avg_regret_final'].min())
    max_val = max(plot_df['gamma_bar_threshold'].max(), plot_df['avg_regret_final'].max())
    ax.plot([min_val, max_val], [min_val, max_val], 'k--', alpha=0.5, label='y = x (perfect adherence)')
...
def plot_gamma_error_histogram(df: pd.DataFrame, figsize: Tuple[int, int] = (10, 6)) -> None:
    """
    ...
    # Add statistics
    ax.text(0.7, 0.9, f'Mean: {violations.mean():.3f}\nStd: {violations.std():.3f}', 
...
def plot_correlation_panels(df: pd.DataFrame, target_cols: Optional[List[str]] = None,
    ...
    if target_cols is None:
        target_cols = ['PT_error', 'ST_error', 'rho_spent_final', 'target_PT', 'target_ST']
    ...
    for i, col in enumerate(available_cols):
        ...
        spearman_corr, spearman_p = stats.spearmanr(plot_data[col], plot_data['regret_ratio'])
        pearson_corr, pearson_p = stats.pearsonr(plot_data[col], plot_data['regret_ratio'])
