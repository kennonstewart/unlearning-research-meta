...
def plot_accountant_comparison(
    ...
    for acc_type in accountant_types:
        ...
        plt.plot(
            ...
            label=f"{acc_type} (n={len(all_curves)})",
            linewidth=2,
        )
        plt.fill_between(
            x,
...
def plot_accountant_summary_stats(summary_file: str, out_path: str) -> None:
    ...
    try:
        ...
        available_metrics = [
            (col, title) for col, title in metrics_to_plot if col in df.columns
        ]
        ...
        for i, (metric_col, metric_title) in enumerate(available_metrics):
            ...
            available_accountants = [
                acc for acc in accountant_order if acc in df["accountant_type"].values
            ]
...
def plot_capacity_vs_noise_tradeoff(summary_file: str, out_path: str) -> None:
    ...
    try:
        ...
        if not all(col in df.columns for col in required_cols):
            print(
                f"Warning: Required columns {required_cols} not found in summary file"
            )
            return
            ...
            if not subset.empty:
                ...
                plt.annotate(
                    ...
                    bbox=dict(
                        ...
                        facecolor=colors.get(acc_type, "white"),
                        alpha=0.7,
                    ),
                    fontsize=9,
                )
    ...
    try:
        ...
        for p in csv_paths:
            ...
            if "N_star_live" in df.columns:
                ...
                ax1.plot(
                    events, df["N_star_live"], alpha=0.7, label=os.path.basename(p)
                )
            ...
            if "m_theory_live" in df.columns:
                events = np.arange(len(df))
                ax2.plot(
                    events, df["m_theory_live"], alpha=0.7, label=os.path.basename(p)
                )
...
def plot_regret_decomp(csv_paths: List[str], out_path: str) -> None:
    ...
    try:
        ...
        for p in csv_paths:
            ...
            if all(
                col in df.columns
                for col in ["regret_static_term", "regret_path_term", "regret_dynamic"]
            ):
        ...
        plt.plot(
            events,
            pad_and_mean(dyn_curves),
            label="Dynamic Regret",
            alpha=0.8,
            linestyle="--",
        )
